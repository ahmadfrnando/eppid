        ...
        <div class="flex flex-wrap items-center justify-between pb-4 space-y-4 flex-column sm:flex-row sm:space-y-0">
            <div class="flex items-center justify-center space-x-2">
                <select wire:model="selectedData" wire:change="UpdatingSearch" id="table-search"
                    ...
                    <option value="null" {{ $selectedData === 'null' ? 'selected' : '' }}>Semua Data</option>
                ...
                <x-button wire:click="cetakPengkeber()" wire:loading.attr="disabled" target="_blank"
                ...
                @foreach ($pengkebers as $pengkeber)
                    <tbody>
                        <tr
                            ...
                            <td class="px-6 py-4">
                                {{ Carbon::parse($pengkeber->created_at)->formatLocalized('%A, %d %B %Y') }}
                            </td>
                            <td class="px-6 py-4">
                                <button class="w-full">
                                    <span
                                        class=" text-xs font-medium me-2 px-2.5 py-0.5 rounded-full {{ $pengkeber->status == 'DITOLAK' ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' : 'bg-teal-100 text-teal-800 dark:bg-teal-900 dark:text-teal-300' }}"
                                        wire:click="openModal({{ $pengkeber->id }})">{{ $pengkeber->status }}</span>
                                </button>
                            </td>
                            <td class="flex px-6 py-4 w-50">
                                <a href="../{{ $pengkeber->buktipengajuan }}" target="_blank">
                                    <span
                                ...
                                <button>
                                    <span
                                    class="inline-flex items-center justify-center w-8 h-8 me-2 text-sm font-semibold text-gray-800 bg-gray-100 rounded-full dark:bg-gray-700 dark:text-gray-300"
                                    wire:click="openModalDoc({{ $pengkeber->id }})">
                ...
                <x-slot name="content">
                    <div class="col-md-12">
                        ...
                        <x-select class="w-full mt-2" wire:model="status">
                            ...
                            <option value="PROSES" class="{{ $status == 'PROSES' ? 'hidden' : '' }}">PROSES</option>
                            <option value="DITERIMA" class="{{ $status == 'DITERIMA' ? 'hidden' : '' }}">DITERIMA
                            </option>
                            <option value="DITOLAK" class="{{ $status == 'DITOLAK' ? 'hidden' : '' }}">DITOLAK
                            </option>
                        </x-select>
                    </div>
                    <div class="col-md-12">
                        ...
                        <x-select class="w-full mt-2" wire:model="pesan">
                            <option value="null" selected disabled>Pilih pesan</option>
                ...
                <x-slot name="content">
                    <div class="gap-2 md:grid-1">
                        <div class="flex">
                            ...
                            @enderror
                            <div wire:loading wire:target="doc" class="text-sm text-blue-500 ms-2">Uploading...</div>
                        </div>
